...
export default function AddBot({ opened, setOpened, onAdd }: { opened: boolean; setOpened: (o: boolean) => void; onAdd: (b: Bot) => void }) {
  const [name, setName] = useState("");
  const [go, setGo] = useState<GoMode>({ t: "Depth", c: 10 });

  function create() {
    if (!name.trim()) return;
    onAdd({ id: genID(), name: name.trim(), go });
    setOpened(false);
    setName("");
  }
  ...
  return (
    <Modal opened={opened} onClose={() => setOpened(false)} title="Add Bot">
      <Stack>
        <TextInput label="Name" placeholder="My Bot" value={name} onChange={(e) => setName(e.currentTarget.value)} />
