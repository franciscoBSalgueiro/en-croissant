...
export function getCardForReview(cards: Card[], options: { random: boolean } = { random: false }): Card | null {
    if (options.random) {
...
export function updateCardPerformance(setCards: React.Dispatch<SetStateAction<Card[]>>, i: number, isRecalled: boolean) {
    setCards((cards) => {
